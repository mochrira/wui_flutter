...
class WuiButton extends StatelessWidget {
  ...
  Color getThemeColor(BuildContext context) {
    switch(theme) {
      case WuiButtonTheme.danger: return const Color(0xFFDB2323);
  ...
  Color getFillColor(BuildContext context) {
    return smooth == true ? Colors.transparent : getThemeColor(context);
  }

  Widget getIconSpacing() {
    return size == WuiButtonSize.normal ? const SizedBox(width: 8) : const SizedBox(width: 16);
  }

  Color getHighlightColor(BuildContext context) {
    if(smooth == true) {
      switch (theme) {
        case WuiButtonTheme.danger: return const Color(0xFFDB2323).withOpacity(.12);
